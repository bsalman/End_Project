		...
		_createClass(Timer, [{
			...
			value: function render() {
				if (this.config.format) {
					...
					$(this.element)[this.html](this.config.attribute,this.totalSeconds);
                    if (this.config.style) {
                        var factor = 100/parseInt(this.config.duration, 10);
                        $(this.element).css( this.config.style, factor*this.totalSeconds+'%' );
				    } 
				} else {
