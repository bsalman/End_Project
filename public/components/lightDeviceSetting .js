...
import {
 ...
 import { changeDeviceStatus, addTimeDevicePost, updateTimeDevicePost, deleteTimeDevicePost } from '../services/api'
const MotionDeviceSetting = ({lightdevice, Id }) => {
  useEffect(() => {
    //console.log(appliancDevices);
    setState({
      ...lightdevice, deleted: false
    })
  }, [lightdevice])
  ...
  const setDeviceStatus = (relationId) => {
    changeDeviceStatus(relationId).then(data => {
  ...
  const onClickSaveBtn = () => {
    ...
    if (state.id === null) {
      addTimeDevicePost(state.startTime, state.stopTime, deviceId, state.selectedDevice, state.active).then(data => {
      ...
      updateTimeDevicePost(state.id, state.startTime, state.stopTime, motionId, state.selectedDevice, state.active).then(data => {
        saveRef.current.classList.add('d-none')
      }).catch(error => {
        console.log('error', error);
      })
          ...
          <div className="row col-xl-3 col-sm-12">
            ...
            <div className="mt-5 col-xl-4 col-sm-4">
              <Label className={`switch ml-auto ${state.active ? 'checked' : ''}`} >
                <Input type="checkbox" onChange={() => { setDeviceStatus(state.id) }} />
              </Label>
            </div>
          </div>
